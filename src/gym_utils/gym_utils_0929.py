...
class ObservationSpace(gym.spaces.Box):
    ...
    def __init__(self):
        super(ObservationSpace, self).__init__(low=-1, high=1, shape=(OBSERVATION_DIM,), dtype=np.float32)
...
class ActionSpace(gym.spaces.Box):
    ...
    def from_gym(self, action, action_p, legal_act_space):
        """
            Convert array-like action (range [-1,1]) to act
        """
        action = np.array(action).flatten().astype(np.float32)
        action_p = np.array(action_p).flatten().astype(np.float32)
        
        act = {"adjust_gen_p": action_p,
               "adjust_gen_v": action * 0.01 }
